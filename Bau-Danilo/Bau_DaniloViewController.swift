...
class Bau_DaniloViewController: UIViewController{
    ...
    func DisponiNumeri(){
        ...
        for i in offset...offset + level - 1{
            ...
            let label1 = UIImageView.init(image: (UIImage(named: String(array[i]) + "DaniloBau.png")))
            label1.frame = CGRectMake(0, 0, 75, 75)
            label1.center = CGPointMake(CGFloat(Float(x)), 250)
            self.view.addSubview(label1)
            arrayImm2.insert(label1, at: cont)
            let label = DraggableImageView.init(image: (UIImage(named: String(array[i]) + "DaniloBau.png")))
            label.frame = CGRectMake(0, 0, 70, 70)
            label.center = CGPointMake(CGFloat(Float(x)), 250)
            label.image = UIImage(named: String(array[i]) + "DaniloBau.png")
            label.tag = i
